...
To enable ~+asdf-mode~ in =.asd= files that run ~lisp-mode~, we need to hook into
~find-file-hook~; it's easier for major modes, which can make use of
~auto-mode-alist~.
