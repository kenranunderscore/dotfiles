   ...
   (list current-prefix-arg
         (completing-read
          ...
          (mapcar (lambda (f) (plist-get f :family))
                  (seq-remove (lambda (f)
                             (string= (plist-get f :family)
                                      +current-font))
                           +fonts)))))
  (let* ((attrs (seq-find (lambda (f)
                          (string= (plist-get f :family)
                                   font))
                        +fonts))
         (default-height (plist-get attrs :default-height))
...
(use-package evil
  ...
  (evil-mode 1)
  :config
  (setq evil-insert-state-cursor '(hbox . 3))
  (general-define-key
   :states 'normal
   "U" 'evil-redo)
  (general-define-key
...
Some of our work projects use ~cljfmt~ to enforce a certain code style. It's a
