...
The following utility function ~+sly-load-project~ slightly simplify the above
process by only needing to be inside the ASDF file with a connection to ~sly~. The
rest is done automatically.

FIXME: find out how to start ~sly~ as part of the process
