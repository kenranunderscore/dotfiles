            ...
            :match-func (lambda (msg)
                          (when msg
                            (string-prefix-p "/ag"
                                             (mu4e-message-field msg :maildir)
                                             t)))
            :vars `((user-mail-address ly-raw string "\"johannes.maier@active-group.de\"")
...
function which I can call whenever these sorts of changes happen,
...
elements.  Also increase org header size."
  (interactive)
  ;; FIXME: colors like `shadow' and `font-lock-comment-face' should
  ;; be set via theme, not here.  It should only amount to
  ;; `fixed-pitch', `:height' and the like here.  Figure out what the
  ;; correct way of saying "use a monospace font here" is; maybe set
  ;; my current font specifically via `:font'?  That would get rid of
  ;; overriding issues with `:inherit' maybe.
  ...
  (set-face-attribute 'org-document-title nil :inherit 'default :height 1.8 :weight 'bold)
