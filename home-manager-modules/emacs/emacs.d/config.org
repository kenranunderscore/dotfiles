...
Custom ELisp files (locally defined Emacs packages for instance)
should be going into =~/.emacs.d/lisp/=, so that has to be in the
=load-path=.
...
everything lazily, so I've created a custom local Emacs package than
contains everything.
