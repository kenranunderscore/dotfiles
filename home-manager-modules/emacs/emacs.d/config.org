...
Install [[https://github.com/jwiegley/use-package][use-package]]; it integrates well with =straight=.
...
#+begin_src emacs-lisp
(setq kenran/want-evil nil)

(defmacro when-evil (body)
  "Only evaluate BODY when `kenran/want-evil' is set."
  (when kenran/want-evil
    body))
#+end_src

#+begin_src emacs-lisp
(defmacro use-package-when-evil (&rest args)
  "Like the normal `use-package' invocation, but disables the
package if `kenran/want-evil' is set."
...
A quick way to jump to this file.
...
is true, then always set the colors anew."
  (when (and kenran/want-evil
             (or force (not kenran/has-set-cursor-colors)))
    (setq kenran/has-set-cursor-colors t)
    (let* ((default-cursor 'box)
           (insert-cursor '(hbar . 5)))
...
(defun kenran/switch-font (font)
  ...
  (let* ((attrs (alist-get font kenran/font-alist))
         ...
         (height (or (plist-get attrs :default-height) 160))
...
- I like having a visual, i.e. colorful and very visible, indicator of
  which =evil= state/mode I'm in.

  So let's try to fiddle with the default mode line to make it work
  the way I like.

#+begin_src emacs-lisp
(defgroup kenran/faces nil
  "A group for custom faces."
  :group 'faces)
...
(defcustom kenran/evil-state-mode-line-format
  '(:eval
      ...
      (cond
       ((eq evil-state 'insert)
        (propertize
         " INSERT "
         'face
         (kenran/with-active-face `(:foreground ,bg :background ,error-fg))))
       ((eq evil-state 'normal)
        (propertize
         " NORMAL "
         'face
         (kenran/with-active-face `(:foreground ,bg :background ,fg))))
       ...
       ((eq evil-state 'visual)
        (propertize
         " VISUAL "
         'face
         (kenran/with-active-face `(:foreground ,bg :background ,(face-attribute 'font-lock-function-name-face :foreground)))))
       ((eq evil-state 'emacs)
        (propertize
         " EMACS "
         'face
         (kenran/with-active-face `(:foreground ,bg :background ,(face-attribute 'font-lock-keyword-face :foreground)))))
       (t "        "))))
...
(setq-default
 mode-line-format
 '(""
   kenran/evil-state-mode-line-format
...
I'm usually using =evil-mode= for vim emulation, with varying degrees
(over time) of how much of =evil-collection= I sprinkle in -- either
nearly everything or nearly nothing.  For now, I'm thinking of going
back to vanilla Emacs bindings to finally /really/ grasp them and see if
it clicks for me.  I cannot put my finger on why exactly, but in my
heart of hearts I'm not a modal editing person, even though I use lots
of it.
...
(defmacro with-leader (&rest args)
  (when-evil
   `(leader ,@args)))
#+end_src
...
(defmacro with-local-leader (&rest args)
  (when-evil
   `(local-leader ,@args)))
#+end_src
...
[[https://depp.brause.cc/dotemacs][wasamasa's Emacs configuration]], specifically the part where I default
to =emacs= mode.  The reason is that (sometimes due to =evil=, other times
=evil-collection=) some buffers, like popups in =special-mode=,  don't
behave the way I'd expect them to.  If this experiment goes well I
think I'll gradually remove =evil= / =evil-motion-state= from more and
more things.  I'm very used to =evil= integration and keybindings in
things like =magit=, =dired= and others, though.  If I end up going
"vanilla" except for editing, I might have to think about how I want
to move consistently so I don't get caught up between =C-n= and =C-p=
versus =C-j= and =C-k=.

#+begin_src emacs-lisp
(use-package-when-evil evil
  :init
...
#+begin_src emacs-lisp
(use-package-when-evil evil-surround
  :after evil
  :config
  (global-evil-surround-mode))
#+end_src

*** 2-character searching/jumping
**** =avy=

After using [[https://github.com/ggandor/leap.nvim][leap.nvim]] in =neovim= for a while now, I really prefer this
way of jumping, as I can get to the correct candidate faster (while
losing out on =;= and =,=, but maybe it's possible to get both).  So let's
give [[https://github.com/abo-abo/avy][avy]] a try.

#+begin_src emacs-lisp
(use-package-when-evil avy
  :after evil
  :config
...
#+begin_src emacs-lisp
(use-package-when-evil evil-org
  :after (evil org)
...
*** Evilify other packages

I have a love/hate relationship with [[https://github.com/emacs-evil/evil-collection][evil-collection]]: on one hand, it
gives all the awesome Emacs packages a more unified and vimish
interface, thus removing the need for a mental switch between, as an
example, using =C-n= or =C-p= for navigation instead of =j= and =k=, but on
the other hand obscuring some default Emacs keybindings of those
packages, effectively hiding some "default" functionality.

In the past I decided to move away from this full evilified Emacs
experience, and follow the doctrine "vim is for editing, everything
else can be used Emacs style".  But it didn't really feel great either
due to the above-mentioned context switches all the time.  So after
spending some time in =neovim= (where everything is "evil" of course)
but missing my precious Emacs, I now give =evil-collection= another try.

#+begin_src emacs-lisp
(use-package-when-evil evil-collection
  :after evil
  :config
  (dolist (feature '(lispy corfu))
    (delete feature evil-collection-mode-list))
...
(with-leader
  ...
  "n e" '(hydra-navigate-errors/body :which-key "error list"))
#+end_src
...
** Integration with =evil-repeat=

Usually when completing something with =corfu= and then using
=evil-repeat= leads to unexpected behavior (see [[https://github.com/minad/corfu/pull/225][this GitHub issue]]).
This is the workaround mentioned there; hopefully it won't break too
often.
...
Now ignore moving through the completion list when determining what
constitutes a repeatable change, and in turn mark insertion and
completion as a repeatable change.
...
(with-local-leader
  ...
  "p" '(json-pretty-print-buffer :which-key "prettify buffer"))
#+end_src
...
A couple of Emacs Lisp functions that help me to make quick changes to
...
somewhat easy to add =import= statements to Haskell files.  It's surely
not perfect, but fits my preferences well; that is, I almost never use
the combinations that are harder to add with this template.  For
instance, adding a qualified import with import list, or an
unqualified one with an alias, would require "tricks".
...
Some =haskell-mode= operations like compiling, or jumping to errors,
interfere with the =evil-repeat= mechanism (that is, =.=).  I try to make
=evil= ignore those.
...
No really necessary to bind this, but why not...
...
(with-local-leader
  ...
  "p" '(sly-mrepl-set-package :which-key "in-package"))
#+end_src
...
#+begin_src emacs-lisp
(use-package-when-evil lispyville
  :hook
