...
[[https://github.com/rails-to-cosmos/danneskjold-theme][danneskjold]] and wish to try it out for a little bit.  Its feel is very
similar to =modus-vivendi= for me.  I especially love its =magit= theme.
...
(use-package magit
  ...
  ;; NOTE: I'll have to try
  ;; `magit-display-buffer-fullframe-status-v1' as well, which
  ;; always opens up the status buffer using the full frame.
...
Time to give =parinfer= another try.  =lispy= / =lispyville= take a while to really
grok, while =parinfer= should theoretically work more nicely with vim
motions by default.
...
(use-package parinfer-rust-mode
  ...
  (with-local-leader
    ...
    "p m" '(parinfer-rust-switch-mode :which-key "switch mode")
    "p t" '(parinfer-rust-toggle-disable :which-key "toggle")))
#+end_src
